...
FlarmDataPath(const TCHAR *name) noexcept
{
...
try {
  ...
  const auto path = FlarmDataPath(_T("xcsoar-flarm.txt"));
...
try {
  ...
  const auto path = FlarmDataPath(_T("flarm-msg-data.csv"));
  FileReader reader{path};
  BufferedReader buffered_reader{reader};
